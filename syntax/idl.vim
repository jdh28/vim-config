...
" Types
syn region  idlD4	contained start="<" end=">"	skipempty skipwhite nextgroup=idlSimpDecl	contains=idlSeqType,idlBaseTypeInt,idlBaseType,idlLiteral,idlOtherType
...
syn keyword idlBaseTypeInt	contained	short long int		skipempty skipwhite nextgroup=idlSimpDecl
...
syn keyword idlOtherType    contained   HRESULT     skipempty skipwhite

" Modules
syn region  idlModuleContent contained start="{" end="}"	fold skipempty skipwhite nextgroup=idlSemiColon contains=idlUnion,idlStruct,idlEnum,idlInterface,idlComment,idlTypedef,idlConst,idlException,idlModule
...
" Interfaces
syn region  idlInterfaceContent contained start="{" end="}"	fold skipempty skipwhite nextgroup=idlSemiColon contains=idlUnion,idlStruct,idlEnum,idlComment,idlROAttr,idlAttr,idlOp,idlOneWayOp,idlException,idlConst,idlTypedef
...
" Enum
syn region  idlEnumContents contained start="{" end="}"		fold skipempty skipwhite nextgroup=idlSemiColon, idlSimpDecl contains=idlId,idlComment
...
" Typedef
syn keyword idlTypedef			typedef			skipempty skipwhite nextgroup=idlBaseType, idlBaseTypeInt, idlSeqType, idlOtherType

" Struct
syn region  idlStructContent contained start="{" end="}" fold skipempty skipwhite nextgroup=idlSemiColon, idlSimpDecl	contains=idlBaseType, idlBaseTypeInt, idlSeqType,idlComment, idlEnum, idlUnion, idlOtherType
...
" Union
syn match   idlColon contained ":"	skipempty skipwhite nextgroup=idlCase,idlSeqType,idlBaseType,idlBaseTypeInt,idlOtherType
...
syn region  idlUnionContent	contained start="{" end="}"		fold skipempty skipwhite nextgroup=idlSemiColon,idlSimpDecl	contains=idlCase
syn region  idlSwitchType	contained start="(" end=")"		fold skipempty skipwhite nextgroup=idlUnionContent
...
syn region idlParams start="\[" end="\]" fold containedin=idlEnumContents,idlModuleContent,idlInterfaceContent,idlStructContent contains=idlString,idlString

syn match   idlArgModifiers  "\[\s*\(in\|out\|retval\)\(\s*,\s*\(in\|out\|retval\)\)*\s*\]" containedin=idlModuleContent

syn region otherFold start="{" end="}" transparent fold
...
if version >= 508 || !exists("did_idl_syntax_inits")
  ...
  HiLink idlType                Type
  HiLink idlParams           PreProc
  HiLink idlArgModifiers     Keyword
